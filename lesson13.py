...
A simple agent that answers questions about the menus-async.csv file.
It reads the CSV data and can answer questions like:
- Where can I get the cheapest coffee in Tampere?
- What restaurants serve vegan options?
- What is the average lunch price?
...
query_agent = Agent(
    ...
    You have access to a CSV database containing information about restaurants, their menu items,
    prices, categories, dietary tags, cuisine types, Google ratings, and addresses.

    The CSV columns are: timestamp, source_image, restaurant_name, dish_name, price, category, dietary_tags, cuisine_type, google_rating, address
...
def add_csv_data(ctx: RunContext[str]) -> str:
    ...
    return f"\n\nHere is the restaurant and menu database:\n\n{ctx.deps}"
