...
def process_image(image_path: Path):
    ...
    try:
        ...
        extraction_result = extraction_agent.run_sync([
            "Extract all menu information from this image.",
            BinaryContent(data=image_data, media_type=media_type),
        ])

        # Enrich the restaurant with rating, address, and cuisine type using the second agent
        ...
        enrichment_result = enrichment_agent.run_sync(
        ...
        enriched_extraction = EnrichedMenuExtraction(
            ...
            cuisine_type=enrichment_result.output.cuisine_type,
            google_rating=enrichment_result.output.google_rating,
            address=enrichment_result.output.address,
            items=extraction_result.output.items
        )

        # Print results
        print(f"   Restaurant: {enriched_extraction.restaurant_name}")
        print(f"      Cuisine: {enriched_extraction.cuisine_type}")
        print(f"      Rating: {enriched_extraction.google_rating}")
        print(f"      Address: {enriched_extraction.address}")
